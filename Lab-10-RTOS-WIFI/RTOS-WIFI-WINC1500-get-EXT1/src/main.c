...
void io_init(void){
     ...
     pmc_enable_periph_clk(BUT_PIO_PLACA);
     pio_configure(BUT_PIO_PLACA, PIO_INPUT, BUT_PIO_PLACA_IDX_MASK, PIO_PULLUP | PIO_DEBOUNCE);
     pio_set_debounce_filter(BUT_PIO_PLACA, BUT_PIO_PLACA_IDX_MASK, 60);
     pio_handler_set(BUT_PIO_PLACA,
     BUT_PIO_PLACA_ID,
     BUT_PIO_PLACA_IDX_MASK,
     PIO_IT_FALL_EDGE,
     but_callback);
     
     pio_enable_interrupt(BUT_PIO_PLACA, BUT_PIO_PLACA_IDX_MASK);
     pio_get_interrupt_status(BUT_PIO_PLACA);
     
     NVIC_EnableIRQ(BUT_PIO_PLACA_ID);
     NVIC_SetPriority(BUT_PIO_PLACA_ID, 4);
}

void formata_get(char *sendbuffer, char *path){
	sprintf(sendbuffer, path);
}
/************************************************************************/
/* callbacks                                                            */
/************************************************************************/
...
static void task_process(void *pvParameters) {
  ...
  while(1){
    ...
    switch(state){
	  ...
	  if( xSemaphoreTake(xSemaphoreBut, 0) == pdTRUE ){
		  	if(status_led){
      ...
      case GET:
      printf("STATE: GET \n");
	  formata_get(g_sendBuffer, MAIN_PREFIX_BUFFER);
      send(tcp_client_socket, g_sendBuffer, strlen((char *)g_sendBuffer), 0);
      state = ACK;
      break;
