...
SemaphoreHandle_t xSemaphoreButOled1;
...
void but_oled1_callback(void) {
    BaseType_t xHigherPriorityTaskWoken = pdFALSE;
    xSemaphoreGiveFromISR(xSemaphoreButOled1, &xHigherPriorityTaskWoken);
    int inc = 100;
...
static void task_but(void *pvParameters) {
    ...
    for (;;) {
        ...
        //     }
        // }
        // if (xSemaphoreTake(xSemaphoreButOled1, 1000)) {
        //     /* atualiza frequencia */
        //     delayTicks += 100;
        //     /* envia nova frequencia para a task_led */
        //     xQueueSend(xQueueLedFreq, (void *)&delayTicks, 10);
        ...
        if (xQueueReceive(xQueueIncDec, &msg, (TickType_t)0)) {
            delayTicks += msg;
			/* envia nova frequencia para a task_led */
            xQueueSend(xQueueLedFreq, (void *)&delayTicks, 10);
...
static void BUT_init(void) {
    ...
    NVIC_SetPriority(BUT_PIO_ID, 5);       // Prioridade 5
    NVIC_SetPriority(BUT_PIO_OLED1_ID, 5); // Prioridade 5
}
...
int main(void) {
    ...
    xSemaphoreButOled1 = xSemaphoreCreateBinary();
    if (xSemaphoreButOled1 == NULL)
        printf("falha em criar o semaforo do OLED \n");
    ...
    xQueueIncDec = xQueueCreate(32, sizeof(uint32_t));
    if (xQueueLedFreq == NULL)
        printf("falha em criar a queue \n");
