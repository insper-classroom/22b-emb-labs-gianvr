...
static void task_proc(void *pvParameters) {
    ...
    while (1) {
        if (xQueueReceive(xQueueADC, &(adc), 1000)) {
            if (contador < 9) {
                leituras[contador] = adc.value;
                contador++;
            } else {
                for (int i = 1; i <= 9; i++) {
                    leituras[i - 1] = leituras[i];
                }
                leituras[9] = adc.value;
                int media_movel = 0;
                for (int i = 0; i <= 9; i++) {
                    media_movel += leituras[i];
                }
                media_movel /= 10;
                xQueueSendFromISR(xQueuePROC, &media_movel, &xHigherPriorityTaskWoken);
            }
        }
    }
}
...
static void task_adc(void *pvParameters) {
    int mediam;
    for (;;) {
        if (xQueueReceive(xQueuePROC, &(mediam), 1000)) {
            printf("ADC media movel: %d \n", mediam);
        }
    }
}
/************************************************************************/
